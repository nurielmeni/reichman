...
<div class="nls-field input <?= isset($wrapperClass) ? $wrapperClass : '' ?>">
  ...
  <div class="relative">
    ...
    <input <?= $id ? 'id="' . $id . '"' : '' ?> type="<?= isset($type) ? $type : 'text' ?>" name="<?= isset($name) ? $name : '' ?>" value="<?= isset($value) ? $value : '' ?>" placeholder="<?= isset($placeHolder) ? $placeHolder : '' ?>" class="border-2 px-2 <?= isset($prepend) ? ' pl-11 rtl:pl-0 rtl:pr-11' : '' ?> <?= isset($class) ? $class : '' ?>" validator="<?= isset($validators) && is_array($validators) ? implode(' ', $validators) : '' ?>" aria-invalid="false" aria-required="<?= $required  ? 'true' : 'false' ?>" <?= isset($autofocus) &&  $autofocus ? 'autofocus' : '' ?> <?= isset($direction) ? 'dir="' . $direction . '"' : '' ?>>
